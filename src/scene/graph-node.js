...
class GraphNode extends EventHandler {
  ...
  /**
 * Helper static method to recurse findOne without calling createTest constantly.
 *
...
static findNode(node, test) {
  ...
  for (let i = 0; i < len; ++i) {
    const result = this.findNode(children[i], test);
    if (result) {
      return result;
    }
  }
  ...
  constructor(name = "Untitled") {
    ...
    // Allocate a slot in the global transform store
    this._slot = transformStore.allocSlot();
  ...
  /**
   * Rebind all proxy objects after the transform store grows (typed arrays are replaced).
   *
   * @private
   */
  _rebindProxies() {
   ...
   */
  get root() {
    let result = /** @type {GraphNode} */ (this);
   ...
   */
  clone() {
    const clone = new (/** @type {new () => this} */ (this.constructor))();
  ...
  destroy() {
    ...
    // Free the SoA store slot
    if (this._slot >= 0) {
      transformStore.nodeRefs[this._slot] = null;
      transformStore.freeSlot(this._slot);
      this._slot = -1;
    }
  ...
  findOne(attr, value) {
    const test = createTest(attr, value);
    return GraphNode.findNode(this, test);
  }
  ...
  findByPath(path) {
    ...
    for (let i = 0, imax = parts.length; i < imax; ++i) {
      const child = result.children.find((c) => c.name === parts[i]);
      if (!child) {
        return null;
      }
      result = child;
   ...
   */
  getLocalEulerAngles() {
    this.localRotation.getEulerAngles(_localEuler);
    return _localEuler;
  }
   ...
   */
  getScale() {
    return this.getWorldTransform().getScale(_worldScale);
  }
  ...
  addChildAndSaveTransform(node) {
    ...
    node.setPosition(
      tmpMat4.copy(this.worldTransform).invert().transformPoint(wPos),
    );
  ...
  _prepareInsertChild(node) {
    ...
    Debug.assert(
      node !== this,
      `GraphNode ${node?.name} cannot be a child of itself`,
    );
    Debug.assert(
      !this.isDescendantOf(node),
      `GraphNode ${node?.name} cannot add an ancestor as a child`,
    );
  }
  ...
  _onInsertChild(node) {
    ...
    // Mirror hierarchy in the SoA store
    transformStore.setParent(node._slot, this._slot);
  ...
  removeChild(child) {
    ...
    // Mirror in SoA store
    transformStore.setParent(child._slot, -1);
        ...
        if (this.scaleCompensation) {
          ...
          if (parentToUseScaleFrom) {
            while (
              ...
              parentToUseScaleFrom.scaleCompensation
            ) {
            ...
            if (parentToUseScaleFrom) {
              ...
              if (parentToUseScaleFrom) {
                parentWorldScale =
                  parentToUseScaleFrom.worldTransform.getScale();
          ...
          if (parent.scaleCompensation) {
            scaleCompensateScaleForParent.mul2(
              parentWorldScale,
